...
CREATE POLICY "Managers and HR can view all profiles" ON public.profiles
    FOR SELECT USING (
        public.has_role(auth.uid(), 'manager') OR public.has_role(auth.uid(), 'hr')
    );
...
CREATE POLICY "HR can view all roles" ON public.user_roles
...
CREATE POLICY "Managers can view all tasks" ON public.tasks
    FOR SELECT USING (public.has_role(auth.uid(), 'manager') OR public.has_role(auth.uid(), 'hr'));

CREATE POLICY "Managers can manage all tasks" ON public.tasks
    FOR ALL USING (public.has_role(auth.uid(), 'manager') OR public.has_role(auth.uid(), 'hr'));

CREATE POLICY "Anyone can create tasks" ON public.tasks
    FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);

-- Company policies - everyone can read
CREATE POLICY "Anyone can view active policies" ON public.company_policies
    FOR SELECT USING (is_active = true);

CREATE POLICY "HR can manage policies" ON public.company_policies
    FOR ALL USING (public.has_role(auth.uid(), 'hr'));
...
CREATE POLICY "Managers and HR can create announcements" ON public.announcements
...
BEGIN
    ...
    -- Create role (default to employee, or use metadata)
